...
export default function Filter() {
  ...
  // 쿼리 스트링 중 major을 설정하는 state 값
  const [majorParam, setMajorParam] = useState<string>(
  ...
  const handleParams = () => {
    ...
    if (
      ...
      newUrl.get("major") !== searchParams.get("major")
    ) {
      ...
      <ul className={style.filter}>
        ...
        <li>
          ...
          >
            ...
            <SelectContent className="bg-white">
              <SelectGroup>
                <SelectLabel className="text-[20px]">전공</SelectLabel>
                {Object.entries(majorCode).map((major) => (
                  <SelectItem
                    className="cursor-pointer text-[20px] hover:bg-slate-100"
                    key={major[1]}
                    value={major[1] || ""}
